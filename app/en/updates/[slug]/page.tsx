...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const article = getPublishedUpdateBySlug("en", (await params).slug);
  if (!article) return {};
  return {
    title: article.seo?.title ?? article.title,
    description: article.seo?.description ?? article.summary,
    alternates: { canonical: `/en/updates/${article.slug}/` },
    robots: "index, follow",
  };
}
...
export default async function UpdateDetailPage({ params }: PageProps) {
  const article = getPublishedUpdateBySlug("en", (await params).slug);
  if (!article) notFound();
  const affectedLevels = getPublishedRelatedLevels("en", article.affectedLevelNumbers);
  return (
